...
- (void)setLastUpdatedTimeKey:(NSString *)lastUpdatedTimeKey
{
    ...
    if (lastUpdatedTime) {
        ...
        // 3.显示日期
        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"%@：%@", NSLocalizedString(@"上次更新", nil),time];
    } else {
        self.lastUpdatedTimeLabel.text = NSLocalizedString(@"暂无更新记录", nil);
    }
}
...
//    [self setTitle:NSLocalizedString(MJ_PULLDOWN, nil) forState:MJRefreshStateIdle];
//    [self setTitle:NSLocalizedString(MJ_RELEASE_REFRESH, nil) forState:MJRefreshStatePulling];
//    [self setTitle:NSLocalizedString(MJ_REFRESHING, nil) forState:MJRefreshStateRefreshing];
}
